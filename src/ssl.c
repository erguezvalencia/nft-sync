...
static int ssl_client_init(struct ssl_client *c, struct ssl_conf *conf)
{
	...
	if (!SSL_CTX_load_verify_locations(c->ctx, "ca/ca.crt", NULL))
		nfts_log(NFTS_LOG_FATAL,"Verify");
	SSL_CTX_set_client_CA_list(c->ctx, SSL_load_client_CA_file("ca/ca.crt"));
	if (!SSL_CTX_use_certificate_file(c->ctx, "ca/client.crt", SSL_FILETYPE_PEM))
		nfts_log(NFTS_LOG_FATAL,"cert");
	if (!SSL_CTX_use_PrivateKey_file(c->ctx, "ca/private/client.key", SSL_FILETYPE_PEM))
...
struct ssl_server *ssl_server_create(struct ssl_conf *conf)
{
	...
	if (!SSL_CTX_load_verify_locations(c->ctx, "ca/ca.crt", NULL))
		nfts_log(NFTS_LOG_FATAL,"Verify");
	SSL_CTX_set_client_CA_list(c->ctx, SSL_load_client_CA_file("ca/ca.crt"));
	if (!SSL_CTX_use_certificate_file(c->ctx, "ca/server.crt", SSL_FILETYPE_PEM))
		nfts_log(NFTS_LOG_FATAL,"cert");
	if (!SSL_CTX_use_PrivateKey_file(c->ctx, "ca/private/server.key", SSL_FILETYPE_PEM))
		nfts_log(NFTS_LOG_FATAL,"key");
	if (!SSL_CTX_check_private_key(c->ctx))
		nfts_log(NFTS_LOG_FATAL,"cert/key");
	SSL_CTX_set_mode(c->ctx, SSL_MODE_AUTO_RETRY);
